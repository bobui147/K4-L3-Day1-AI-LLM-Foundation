...
def compare_models(prompt: str) -> dict:
    ...
    # Tính chi phí ước tính cho GPT-4o (output tokens)
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # Tra bảng giá với fallback cho model lạ (như NVIDIA NIM)
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
...
def streaming_chatbot() -> None:
    ...
    print("Chatbot streaming (gõ 'quit' hoặc 'exit' để thoát)")
    print("-" * 50)
    ...
    while True:
        ...
        # Ghép messages = history + tin nhắn mới
        ...
        # Cắt history còn 3 lượt cuối (6 messages)
        history = history[-6:]
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise  # Hết lượt retry, ném lỗi cuối cùng
            # Exponential backoff: 0.1 * 2^0, 0.1 * 2^1, 0.1 * 2^2...
...
def run_assistant(
    ...
    while True:
        ...
        # Bước 3: Ghép messages với system prompt đứng đầu
        messages = ([{"role": "system", "content": persona}]
                    + history
                    + [{"role": "user", "content": user_msg}])
        
        # Bước 4: Gọi API với retry và streaming
        ...
        # Bước 6: Cập nhật history và cắt còn 6 messages
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]
        
        # Bước 7: Cộng dồn thống kê
        num_turns += 1
        total_tokens += count_tokens(user_msg, OPENAI_MODEL)
        total_tokens += count_tokens(reply, OPENAI_MODEL)
